...
Scope {
    ...
    CustomShortcut {
        ...
        description: "Bật/tắt popup đổi hình nền"
        onPressed: {
            if (root.hasFullscreen)
                return;
            WallpaperSwitcherWindow.toggle(ShellState.forActive()?.modelData);
        }
    }
    ...
    IpcHandler {
        function open(): void {
            WallpaperSwitcherWindow.open(ShellState.forActive()?.modelData);
        }

        function close(): void {
            WallpaperSwitcherWindow.close();
        }
